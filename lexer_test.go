...
func TestLexer(t *testing.T) {
	testCases := []struct {
		...
		{
			"array is contained by other array",
			"array_field <@(20 AND 30)",
			[]item{
				newItem(itemString, "array_field"),
				tSpace,
				newItem(itemContainmentOperator, "<@"),
				newItem(itemLeftParen, "("),
				newItem(itemString, "20"),
				tSpace,
				newItem(itemAnd, "AND"),
				tSpace,
				newItem(itemString, "30"),
				newItem(itemRightParen, ")"),
				tEOF,
			},
		},
		{
